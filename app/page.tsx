...
export default function Home() {
  // --- Mounted-gate: nincs early return! ---
  ...
  /** Reset mindenre – a logó kattintására hívjuk */
  const resetAll = () => {
    setDob("");
    setSerial("100");
    setGenId(null);
  ...
  const onGenerate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setGenId(`Hiba: ${err?.message ?? String(err)}`);
    } finally {
      setLoadingGen(false);
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Pl. "000" tiltott, vagy CDV maradék 10 → új sorszám kell
      setPreviewId(null);
      setPreviewError(err?.message ?? "Előnézet nem képezhető a megadott adatokkal.");
    }
  }, [dob, serial]);

  /* -------------------------------------
     ÚJ: Automatikus ellenőrzés 10 számnál
     ------------------------------------- */
  useEffect(() => {
    if (!valInput) return;
    const clean = valInput.replace(/\D/g, "");
    // 200 ms debounce, csak akkor fut, ha pontosan 10 számjegy van
    let t: any;
    if (clean.length === 10) {
      t = setTimeout(() => {
        // "Enter nélkül" lefuttatjuk a validációt
        onValidate();
      }, 200);
    } else {
      // töröljük a korábbi üzenetet, ha már nem 10 jegyű
      setValMsg(null);
    }
  ...
  return (
    <main
      className={`mx-auto max-w-5xl px-4 py-10 ${mounted ? "" : "invisible"}`}
      suppressHydrationWarning
    >
      {/* Fejléc logóval – logó gombként, reset funkcióval */}
      ...
      {/* Két funkció kártya – teljes oszlopszélességben */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section className="relative w-full rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50 to-blue-100 p-5 shadow-md transition hover:scale-[1.02] hover:shadow-lg dark:border-slate-600 dark:from-slate-800 dark:to-slate-700">
          ...
          <h2 className="mb-1 text-lg font-medium text-gray-800 dark:text-gray-200">Generálás</h2>
          ...
          <form onSubmit={onGenerate} className="mt-4 grid gap-3" suppressHydrationWarning>
              ...
              {tooltipVisible && (
                <div
                  ...
                  className={`absolute left-1/2 top-full z-10 mt-2 w-56 -translate-x-1/2 rounded-md bg-gray-900 p-2 text-xs text-white shadow-lg transition duration-200 ease-out
                    ...
                    dark:bg-black`}
                >
                  A sorszám az azonos napon születettek megkülönböztetésére szolgál.
                  Alapértelmezett: 100. A „000” tiltott.
                </div>
              )}
        ...
        <section className="relative w-full rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50 to-blue-100 p-5 shadow-md transition hover:scale-[1.02] hover:shadow-lg dark:border-slate-600 dark:from-slate-800 dark:to-slate-700">
          ...
          <h2 className="mb-1 text-lg font-medium text-gray-800 dark:text-gray-200">Ellenőrzés</h2>
          <p className="mb-4 text-center text-sm text-gray-700 dark:text-gray-300">
          ...
          <form
            onSubmit={onValidate}
            className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-[1fr_auto]"
            suppressHydrationWarning
          >
            <input
              inputMode="numeric"
