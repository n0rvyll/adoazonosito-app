...
export default function TaxNumberTool() {
  // ÁLLAPOTOK
  ...
  const onValidate = async (e?: React.FormEvent) => {
    ...
    if (!res.valid) {
      setValMsg({
        text: `Érvénytelen: ${res.reason ?? "ismeretlen ok"}`,
        success: false,
      });
    } else {
      setValMsg({
        text: `Az adóazonosító jel érvényes. A születési idő visszafejtve: ${fmt(
          res.birthDate!
        )}`,
        success: true,
      });
    ...
    try {
      ...
      const msg =
        err instanceof Error
          ? err.message
          : "Előnézet nem képezhető a megadott adatokkal.";
      setPreviewId(null);
      setPreviewError(msg);
    }
  }, [dob, serial]);
  ...
  const LoadingOverlay = ({ text }: { text: string }) => (
    <div className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl bg-white/80 backdrop-blur-sm dark:bg-slate-800/80">
      <div className="animate-pulse font-medium text-blue-700 dark:text-blue-300">
        {text}
      </div>
  ...
  return (
    <>
      {/* NINCS <SEOHead /> és NINCS next/head */}
      ...
      <section className="mb-8 mx-auto max-w-xl rounded-xl border border-blue-200 bg-blue-50 p-5 text-sm leading-relaxed text-justify shadow-md">
        ...
        <ol className="mt-2 list-inside list-decimal space-y-1">
          <li>
            Első számjegy: mindig <code>8</code>.
          </li>
          <li>
            2–6. számjegyek: születési dátum és 1867.01.01. között eltelt napok
            száma.
          </li>
          <li>
            7–9. számjegyek: sorszám (alapértelmezett <code>100</code>, „000”
            tiltott).
          </li>
          <li>
            10. számjegy: ellenőrző szám (mod 11; ha maradék 10 → új sorszám
            szükséges).
          </li>
        </ol>
      </section>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section className="relative w-full rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50 to-blue-100 p-5 shadow-md transition hover:scale-[1.02] hover:shadow-lg dark:border-slate-600 dark:from-slate-800 dark:to-slate-700">
          ...
          <p className="mb-4 text-center text-sm text-gray-700 dark:text-gray-300">
            Itt generálható az adóazonosító jel a születési dátum és sorszám
            alapján.
          </p>
              ...
              {tooltipVisible && (
                <div
                  ...
                  className={`absolute left-1/2 top-full z-10 mt-2 w-56 -translate-x-1/2 rounded-md bg-gray-900 p-2 text-xs text-white shadow-lg transition duration-200 ease-out
                    ...
                    dark:bg-black`}
                >
                  A sorszám az azonos napon születettek megkülönböztetésére
                  szolgál. Alapértelmezett: 100. A „000” tiltott.
                </div>
              )}
        ...
        <section className="relative w-full rounded-xl border border-gray-200 bg-gradient-to-br from-blue-50 to-blue-100 p-5 shadow-md transition hover:scale-[1.02] hover:shadow-lg dark:border-slate-600 dark:from-slate-800 dark:to-slate-700">
          ...
          <p className="mb-4 text-center text-sm text-gray-700 dark:text-gray-300">
            Itt ellenőrizhető az adóazonosító jel, és visszafejthető belőle a
            születési idő.
          </p>
          ...
          >
            ...
            />
            {/* Ha külön gomb kellene, ide tehetsz <button type="submit">Ellenőrzés</button>-t,
                de az automatikus (10 számjegy → 200ms) ellenőrzés már működik. */}
          </form>
